...
public class SyncRoomManager {
	...
	/**
	 * Represents a client’s request to start streaming her local media to
	 * anyone inside the room. This is done by initiating the media connection
	 * from the server-side (generate the SDP offer) and must be followed by
	 * processing the SDP answer from the client in order to establish the
	 * streaming.
	 * 
