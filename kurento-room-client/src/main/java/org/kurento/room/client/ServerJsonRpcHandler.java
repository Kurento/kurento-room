...
public class ServerJsonRpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	private Notification participantSendMessage(Transaction transaction,
		...
		String room =
				JsonRoomUtils.getRequestParam(request,
	...
	private Notification roomClosed(Transaction transaction,
			Request<JsonObject> request) {
		String room =
				JsonRoomUtils.getRequestParam(request,
						ProtocolElements.ROOMCLOSED_ROOM_PARAM, String.class);
	...
	private Notification participantUnpublished(Transaction transaction,
		...
		String name =
				JsonRoomUtils.getRequestParam(request,
						...
						String.class);
		ParticipantUnpublishedInfo eventInfo =
				new ParticipantUnpublishedInfo(name);
		log.debug("Recvd participant unpublished event {}", eventInfo);
		return eventInfo;
	}

	private Notification participantPublished(Transaction transaction,
			Request<JsonObject> request) {
		String id =
				JsonRoomUtils.getRequestParam(request,
		...
		JsonArray jsonStreams =
				JsonRoomUtils.getRequestParam(request,
						ProtocolElements.PARTICIPANTPUBLISHED_STREAMS_PARAM,
						JsonArray.class);
		...
		while (streamIt.hasNext())
			streams.add(JsonRoomUtils.getResponseProperty(streamIt.next(),
					...
					String.class));
		ParticipantPublishedInfo eventInfo =
				new ParticipantPublishedInfo(id, streams);
		log.debug("Recvd published event {}", eventInfo);
		return eventInfo;
	}

	private Notification participantEvicted(Transaction transaction,
			Request<JsonObject> request) {
	...
	private Notification participantLeft(Transaction transaction,
			Request<JsonObject> request) {
		String name =
				JsonRoomUtils.getRequestParam(request,
						ProtocolElements.PARTICIPANTLEFT_NAME_PARAM,
						String.class);
	...
	private Notification participantJoined(Transaction transaction,
			Request<JsonObject> request) {
		String id =
				JsonRoomUtils.getRequestParam(request,
	...
	private Notification mediaError(Transaction transaction,
			Request<JsonObject> request) {
		String description =
				JsonRoomUtils.getRequestParam(request,
	...
	private Notification iceCandidate(Transaction transaction,
			Request<JsonObject> request) {
		...
		String sdpMid =
				JsonRoomUtils.getRequestParam(request,
						ProtocolElements.ICECANDIDATE_SDPMID_PARAM,
						String.class);
		int sdpMLineIndex =
				JsonRoomUtils.getRequestParam(request,
		...
		String endpoint =
				JsonRoomUtils.getRequestParam(request,
						ProtocolElements.ICECANDIDATE_EPNAME_PARAM,
						String.class);

		IceCandidateInfo eventInfo =
				new IceCandidateInfo(iceCandidate, endpoint);
